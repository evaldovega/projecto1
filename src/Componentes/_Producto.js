...
class Producto extends React.Component{
    ...
    buscarOrden=()=>{
        global.BD.find({
            ...
            if(result.docs.length>0){
                ...
                const p=orden.products.find(p=>p.id==this.props.route.params.producto.id)
                if(p){
    ...
    ingredienteAgregado=(id)=>{
        const producto=this.props.productos.find(p=>p.id==this.props.route.params.producto.id)
        if(producto){
    ...
    render(){
        ...
        const {images,name,extras,ingredients}=this.props.route.params.producto
        return (
          ...
          >
            ...
            <ScrollView showsVerticalScrollIndicator={false} onScroll={({nativeEvent})=>{
                ...
                <View style={{flex:1,backgroundColor:'#ffff',borderTopLeftRadius:16,borderTopRightRadius:16,padding:16,marginTop:200,elevation:3}}>
                    <Text h3 style={{color:COLOR_PRIMARY}}>{name}</Text>
                    {this.renderIngredientes(ingredients)}
                    {this.renderExtras(extras)}
                    <Text>Instrucciones Especiales</Text>
                    <TextInput multiline={true} value={this.instrucciones()} onChangeText={(t)=>this.props.ingresarInstrucciones(this.props.route.params.producto,t)} numberOfLines={4} underlineColorAndroid='#ffff' style={{borderColor:'silver',borderWidth:1,borderRadius:16}}/>
                </View>
            </ScrollView>
            ...
            <LinearGradient colors={['#fff', '#BFC9CA']} style={{height:100,width:'100%',flexDirection:'row',justifyContent:'space-between',alignItems:'center',paddingHorizontal:16}} >
                        ...
                        <View style={{flexDirection:'row'}}>
                            ...
                            <Text h4 style={{marginHorizontal:16}}>{this.cantidad()}</Text>
                        ...
                        <CurrencyFormat value={this.total()} displayType={'text'} thousandSeparator={true}   prefix={'$'} renderText={value => <Text h5>{value}</Text>} />

                        <Button buttonStyle={{backgroundColor:COLOR_PRIMARY,borderRadius:32,paddingHorizontal:16}} icon={<Icon name='cart-plus' size={24} color='#ffff'/>} title=' Añadir' />
