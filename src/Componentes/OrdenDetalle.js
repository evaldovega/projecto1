...
class OrdenDetalle extends React.Component {
  ...
  conectarSocket = async () => {
    ...
    socket.on('connect', () => {
      ...
      socket.emit(
        'join',
        API_PROJECT_NAME + '_messages_orders_' + this.props.id,
      );
    });
    ...
    socket.on('message', (m) => {
      ...
      if (
        parseInt(m.order_id) == parseInt(this.props.route.params.id) &&
        m.author_id != this.props.id
      ) {
        this.setState((state) => {
          state.mensajes.push({
            comment: m.comment,
  ...
  enviarComentario = async (tipo, file = null) => {
    if (!this.state.enviando_mensaje) {
      ...
      try {
        global.ordering
          .post('/orders/' + this.props.route.params.id + '/messages', data)
          .then((r) => {
  ...
  cargarMensajes = async () => {
    const token = await AsyncStorage.getItem('token');
    fetch(
      'https://apiv4.ordering.co/v400/es/' +
        API_PROJECT_NAME +
        '/orders/' +
        this.props.route.params.id +
        '/messages',
    ...
    if (m.type == 2) {
      return (
        <View
          style={{
            padding: 16,
            backgroundColor:
              this.props.id != m.author_id ? COLOR_PRIMARY : COLOR_ACCENT,
            margin: 8,
            borderRadius: 16,
            alignSelf: this.props.id == m.author_id ? 'flex-end' : 'flex-start',
          }}>
          {this.props.id != m.author_id ? (
            <Text style={{color: '#ffff', fontWeight: 'bold', fontSize: 12}}>
              {m.author.name}
            </Text>
          ) : (
            <></>
      ...
      return (
        <View
          ...
          <Image
            style={{width: '100%', aspectRatio: 4 / 3, height: undefined}}
            source={{uri: m.source}}
          />
          <Text style={{color: '#ffff', fontSize: 14}}>{m.comment}</Text>
        </View>
  ...
  renderMensajes = () => {
    ...
    return this.state.mensajes.map((m) => {
      return <>{this.renderComentario(m)}</>;
    });
  };
      ...
      <View style={{flex: 1, backgroundColor: '#ffff'}}>
        ...
        <View
          ...
          <TouchableOpacity
            ...
            style={{
              flex: 1,
              borderBottomColor: this.state.tab == 1 ? COLOR_PRIMARY : '#ffff',
              borderBottomWidth: 2,
            }}>
            <Text style={{fontSize: 18, textAlign: 'center', paddingBottom: 8}}>
              Pedido
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            ...
            style={{
              flex: 1,
              borderBottomWidth: 2,
              borderBottomColor: this.state.tab == 2 ? COLOR_PRIMARY : '#ffff',
            }}>
            <Text style={{fontSize: 18, textAlign: 'center', paddingBottom: 8}}>
              Mensajes
            </Text>
        ...
        <View
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={{paddingHorizontal: 42}}>{this.renderTotal()}</View>
          </ScrollView>
        </View>
        ...
        <KeyboardAvoidingView
          ...
          <View
            ...
            <Input
              disabled={this.state.enviando_mensaje}
            ...
            <Button
              disabled={this.state.enviando_mensaje}
              buttonStyle={{backgroundColor: COLOR_PRIMARY, marginRight: 8}}
              onPress={() => this.enviarComentario(2)}
              icon={
                <Icon
                  name="paper-plane"
                  type="font-awesome"
            ...
            <Button
              ...
              onPress={() => this.setState({mostrar_opciones_camara: true})}
              title=""
            />
          </View>
